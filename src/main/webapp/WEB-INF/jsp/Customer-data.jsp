...
<!--         <div class="datagrid-toolbar-a">
        ...
        <div class="datagrid-toolbar-a">
            <a id="btn6" href="/eastCMS/client/queryToExcel?khcjlx=0"  class="easyui-linkbutton" data-options="iconCls:'icon-print'">数据导出</a>
        </div>
    <div class="datagrid-toolbar-a">
...
<script type="text/javascript">
    ...
    $(function(){
       ...
       // $("input[name='historySearchTime']").val(historyTimeString);11111111111111
       ...
       //$("input[name='nowSearchTime']").val(nowTimeString);
        ...
        $("#dg").datagrid({
            ...
            rownumbers: true,
            columns: [[
                /* {field : 'cid',title : 'IDs',checkbox : true,width : 8,align : 'left'}, */
                {field: 'cid', title: '编号', width: 50, align: 'left',hidden:true},
                {field: 'cname', title: '姓名', width: 100, align: 'left' },
                {field: 'csex', title: '性别', width: 50, align: 'left',formatter:function(value, row, index){
                    var type="";
                    if(value==0){
                        type="男";
                    }
                    ...
                    return type;}},
                {field: 'cphone', title: '手机号', width: 150, align: 'left'},
                {field: 'ccard', title: '身份证号', width: 100, align: 'left'},
                {field: 'cweixin', title: '微信号', width: 100, align: 'left'},
               /*  {field: 'khwxnc', title: '微信名', width: 100, align: 'left',hidden:true}, */
                {field: 'cqq', title: 'QQ号', width: 100, align: 'left'},
               /*  {field: 'khqqnc', title: '昵称', width: 100, align: 'left'}, */
                {field: 'stauts', title: '客户类型', width: 80, align: 'left',formatter:function(value, row, index){
                    var type="";
                    if(value==1){
                        type="一般客户";
                    }
             ...
             /*    {field: 'memberId', title: '负责人', width: 100, align: 'left',hidden:true},
                ...
                {field: 'ccreatetime', title: '归档日期', width: 180, align: 'left' ,formatter:function(value, row, index){
       ...
       $('#dg').datagrid('getPager').pagination({
        ...
        //添加客户
        $("#btn1").click(function(){
            createwindow("添加客户", "/eastCMS/pages/Customer-add",600,350);
        });
        //修改客户
        $("#btn2").click(function(){
            updata();
       ...
       //导入
        ...
        $("#btn10").click(function(){
            ...
            if(rows.length==0){
                ...
                createnewwindow("移交客户", "/eastCMS/pages/Customer-allot", 600, 350);
            }
        });
        $("#btn8").click(function(){
            var name=$("#khmc").val(),
    ...
    function truns(){
        ...
        if(rows.length==0){
            ...
            createTrunWindow('成交客户','/eastCMS/pages/Customer-trun-update',600,350);
        }
    }
...
function updata(){
    ...
    if(rows.length>1||rows.length==0){
        ...
        createwindow("修改客户", "/eastCMS/pages/Customer-update/"+rows[0].cid,600,350);
    }
}
...
function trace(){
    ...
    if(rows.length>1||rows.length==0){
        ...
        createnewwindow("客户追踪", "/eastCMS/pages/Customer-log",800,520);
    }
}
...
function deletes(){
    ...
    if(rows.length==0){
        ...
        $.messager.confirm('确定', '你确定要删除吗?', function (r) {
            if (r) {
                $.post('/eastCMS/client/batchDelete/'+rows[0].cid, {"ids":JSON.stringify(ids)}, function (result) {
                    //alert(result);
                    if (result.code == 200) {
                        console.log(result);
...
function imports(){
    createwindow("导入", "/eastCMS/pages/imports",400,300);
}
...
function exports(){
    ...
    function tbdata(pageIndex,pageSize){
        ...
        $.ajax({
            url:'/eastCMS/client/getList?pageIndex='+pageIndex+'&pageSize='+pageSize,
            type:'POST',//OR GET
            data:data,
            dataType:'json',
            success:function(data){
